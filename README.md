...
>
> **Auto-join on peer-sight (+ tap to ensure-connected).** When the Mac discovers a Windows host over BLE it **auto-joins** that host's hotspot on a detached worker (GATT read → `joinNetwork` → Hello), guarded by `attemptInFlight` + `helloSent` + a `kPeerRetrySec` cooldown so repeated sightings don't hammer the join. This is required because Windows is the host and *cannot* initiate the Mac's Wi-Fi join — without auto-join, a Windows-initiated transfer to the Mac would wait forever for a Hello that never comes. Tapping a device still calls `bettersend_connect_peer`, which reports progress via `ConnectStatusCallback` (`0` connecting → `1` joined → `2` ready, or `3` failed) and drives the "Connecting…" spinner; if auto-join already connected, the tap returns ready immediately, otherwise it serves as a manual retry. The shared `peersByName` guards keep auto-join and a tap from racing a double-join.
>
